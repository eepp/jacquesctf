...
HelpView::HelpView(const Rect& rect, const Stylist& stylist) :
...
void HelpView::_buildRows()
{
    _rows = {
        ...
        _KeyRow {"e", "Cycle height of \"Event records\" frame"},
        ...
        _KeyRow {"w", "Packet data: make bytes/row count a power of two or anything"},
        ...
        _KeyRow {"E", "Packet data: emphasize or not first region of event record"},
        _KeyRow {"s", "Event records: cycle format of length columns"},
        _KeyRow {"t", "Event records: cycle format of timestamp column"},
        ...
        _KeyRow {"Home", "Go to first region of packet"},
        _KeyRow {"End", "Go to last region of packet"},
        _KeyRow {"C", "Go to first region of packet context"},
        ...
        _KeyRow {":", "Go to offset within data stream file"},
        _KeyRow {"$", "Go to offset within packet (bytes)"},
        _KeyRow {"N, *", "Go to event record with timestamp (ns from origin)"},
        ...
        _SubSectionRow {"Presentation"},
        _KeyRow {"s", "Cycle format of length columns"},
        _KeyRow {"t", "Cycle format of timestamp columns"},
        ...
        _KeyRow {"Enter", "Search if input isn't empty, else cancel"},
    ...
    for (const auto& row : _rows) {
        if (const auto ssRow = boost::get<_SearchSyntaxRow>(&row)) {
            _ssRowFmtPos = std::max(_ssRowFmtPos, static_cast<Size>(ssRow->descr.size() + 4));
        }
    }

    for (const auto& row : _rows) {
        if (const auto sectRow = boost::get<_SectionRow>(&row)) {
            _longestRowWidth = std::max(_longestRowWidth, static_cast<Size>(sectRow->title.size()));
        } else if (const auto sectRow = boost::get<_SubSectionRow>(&row)) {
            _longestRowWidth = std::max(_longestRowWidth, static_cast<Size>(sectRow->title.size()));
        } else if (const auto keyRow = boost::get<_KeyRow>(&row)) {
            _longestRowWidth = std::max(_longestRowWidth,
                                        static_cast<Size>(keyWidth + keyRow->descr.size() + 2));
        } else if (const auto textRow = boost::get<_TextRow>(&row)) {
            _longestRowWidth = std::max(_longestRowWidth, static_cast<Size>(textRow->line.size()));
        } else if (const auto ssRow = boost::get<_SearchSyntaxRow>(&row)) {
            _longestRowWidth = std::max(_longestRowWidth,
                                        static_cast<Size>(ssRow->fmt.size() + _ssRowFmtPos));
        }
    }
}
...
void HelpView::_drawRows()
{
    ...
    for (Index rowNum = 0; rowNum < this->contentRect().h; ++rowNum) {
        ...
        if (const auto sectRow = boost::get<_SectionRow>(&row)) {
            ...
            this->_stylist().std(*this);
            this->_moveAndPrint({startX + keyWidth, y}, "%s", keyRow->descr.c_str());
        } else if (const auto textRow = boost::get<_TextRow>(&row)) {
            this->_stylist().std(*this);
            this->_moveCursor({startX + 2, y});
